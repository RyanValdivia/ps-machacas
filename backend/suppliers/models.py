...
class Supplier(models.Model):
    ...
    provDirec = models.TextField(verbose_name="Address", blank=True, null=True)
    ...
    provCiu = models.CharField(max_length=100, choices = DEPARTAMENTO_CHOICES, verbose_name="City", blank=True, null=True)
    ...
    def clean(self):
        ...
        # Validacion RUC (solo si se proporciona)
        if self.provRuc and (not self.provRuc.isdigit() or len(self.provRuc) != 11):
            raise ValidationError({
                'provRuc': 'El RUC debe tener exactamente 11 dígitos numéricos'
            })
        
        # Validacion phone (solo si se proporciona)
        if self.provTele and (not self.provTele.isdigit() or len(self.provTele) != 9):
            raise ValidationError({
